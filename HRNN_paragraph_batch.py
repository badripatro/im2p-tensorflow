...
import json
'''
2017-08-10 00:33:57.008996: W tensorflow/core/platform/cpu_feature_guard.cc:45] The TensorFlow library wasn't compiled to use SSE4.1 instructions, but these are available on your machine and could speed up CPU computations.
2017-08-10 00:33:57.009024: W tensorflow/core/platform/cpu_feature_guard.cc:45] The TensorFlow library wasn't compiled to use SSE4.2 instructions, but these are available on your machine and could speed up CPU computations.
2017-08-10 00:33:57.009032: W tensorflow/core/platform/cpu_feature_guard.cc:45] The TensorFlow library wasn't compiled to use AVX instructions, but these are available on your machine and could speed up CPU computations.
2017-08-10 00:33:57.009039: W tensorflow/core/platform/cpu_feature_guard.cc:45] The TensorFlow library wasn't compiled to use AVX2 instructions, but these are available on your machine and could speed up CPU computations.
2017-08-10 00:33:57.009046: W tensorflow/core/platform/cpu_feature_guard.cc:45] The TensorFlow library wasn't compiled to use FMA instructions, but these are available on your machine and could speed up CPU computations.
('loss', <tf.Tensor 'add_371:0' shape=() dtype=float32>)
 #to avoid this error i have add this 'os.environ['TF_CPP_MIN_LOG_LEVEL']='2'"'
...
class RegionPooling_HierarchicalRNN():
    def __init__(self, n_words,
        ...
        self.word_LSTM = tf.nn.rnn_cell.MultiRNNCell([lstm_cell(wordRNN_lstm_dim) for _ in range(2)], state_is_tuple=True)
    ...
    def build_model(self):
        ...
        print ("Start build model:") # 150 it is problem, python version 2.7.12, in server 148not problem with python version 2.7.6
        #----------------------------------------------------------------------------------------------
        ...
        for i in range(0, self.S_max):
            ...
            for j in range(0, self.N_max):
                ...
                #print("current_embed get_shape() ",current_embed.get_shape())
                #print("word_state get_shape() ",word_state)
                #print('i',i)
                with tf.variable_scope('word_LSTM'):
    ...
    def generate_model(self):
        ...
        print ("Start build the generation model: ") # changed like line 132, priviously it was like print 'Start build the generation model: '
        ...
        for i in range(0, self.S_max):
            ...
            for j in range(0, self.N_max):
                ...
                #print("type 1 current_embed get_shape() ",current_embed.get_shape())
                #print("type 1  word_state get_shape() ",word_state)
                with tf.variable_scope('word_LSTM'):
                    word_output, word_state = self.word_LSTM(current_embed, word_state)
...
def train():
    ...
    path=json.load(open('imgs_train_path.json', 'r'))
    train_imgs_full_path_lists = path['images_path']    #open('./densecap/imgs_train_path.txt').read().splitlines()
    ...
    #     saver = tf.train.Saver(max_to_keep=500, write_version=1)
        # train_op = tf.train.AdamOptimizer(learning_rate, use_locking=False, name='Adam').minimize(tf_loss,
        #                                                                                         global_step=None,
        #                                                                                         var_list=None,
